...
describe("プロセスを起こす試験の枠", () => {
  ...
  it("bin/ と loop/ の試験は、実測から導いた枠で走る", () => {
    ...
    // git と loop-stall を **9 回**起こし、1 回あたり 0.22〜1.00 秒かかる（実測）。
  ...
  it("枠は、いちばん重い試験の見積もりより大きい", () => {
    // **数字を書き写さない。** 見積もりが変われば枠も変わる形にしておく
    expect(SCRIPT_TEST_TIMEOUT_MS).toBeGreaterThan(9 * 1000);
  });
});
