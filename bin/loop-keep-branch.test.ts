...
describe("bin/loop-keep-branch", () => {
  ...
  it("掴まれていて、しかも進める必要があるなら、黙って成功しない", () => {
    // **倒す先は 2 つある。** **「掴まれていたら全部 [INFO]」にすると、
    // 本当に失われる場合まで黙って通す**——**その周回の commit は、どこからも辿れない。**
    const root = workspace();
    expect(git(["switch", "--quiet", "-c", BRANCH], root).status).toBe(0);
    const held = `${root}-worker-a`;
    expect(git(["worktree", "add", "--detach", "--quiet", held, BRANCH], root).status).toBe(0);
    ...
    expect(kept.status, "失われるのに、成功を返している").toBe(2);
    expect(kept.stderr).toContain("[FAIL]");
  });
