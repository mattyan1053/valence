...
describe("着手を進められる in-progress を数える", () => {
  ...
  it("著者が空の PR が先頭にあっても、抜けない", () => {
    // **削除済みアカウントの PR で `.author.login` は空になる** (#559 のレビュー 3 周目)。
    // **bash は空文字を連想配列の添字にできず**、**`set -e` が無くてもその場で
    // `for` ごと抜ける**——**残りの Issue は数えられないまま、`count` がそこまでの
    // 値で出る。** **先頭で踏めば 0 件**であり、**実装中の Issue まで「作業なし」に
    // なって全ループが止まる。**
    //
    // **入れた手当てが届いていない形である**——**「判らないときは外へ倒す」の行は
    // あるのに、そこへ入力が来ない。**
    const listed = run(
      [501, 503],
      lines([
        { number: 900, branch: "fix/501-x", author: "", labels: WAITING },
        { number: 600, branch: "fix/503-y" },
      ]),
    );

    expect(listed.status, "落ちている").toBe(0);
    expect(listed.stdout, "空の著者で抜けている").toBe("2");
    // **落とした PR を言う。** **黙って落とすと、数が合わない理由に辿り着けない**
    // ——**この行が無ければ、外した事実はどこにも残らない。**
    expect(listed.stderr, "落とした PR を言っていない").toContain("#900");
  });

  it("著者が空の PR が途中にあっても、そのあとの行を読む", () => {
    // **先頭だけを見ると、抜けた残りが数えられていないことを見落とす**
    // ——**同じ Issue の、そのあとの行**が読まれるかを見る。
    const listed = run(
      [501, 503],
      lines([
        { number: 600, branch: "fix/503-y" },
        { number: 900, branch: "fix/501-x", author: "", labels: WAITING },
        { number: 601, branch: "fix/501-z" },
      ]),
    );

    // #501 は 601（ふつうの open PR）を進められる。#503 も 600 を進められる
    expect(listed.status, "落ちている").toBe(0);
    expect(listed.stdout, "空の著者より後ろの行を読んでいない").toBe("2");
  });
