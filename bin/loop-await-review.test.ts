...
describe("bin/loop-await-review", () => {
  ...
  function withSlowReviews(delaySec: number, ...responses: string[]): void {
    ...
    const script = [
      "#!/usr/bin/env bash",
      // **応答の一覧を尋ねているか。** ここで確かめないと、**既定の出力（レビューが
      // 成立したものだけ）を見ていても緑**になる——**SHA を持たない応答は既定から
      // 落ちる**ので、**待つのをやめる条件が「レビューが成立したか」に戻る**（#78）
      'if [[ $* != *"--all"* ]]; then',
      '  echo "スタブ: 応答の一覧（--all）で尋ねていない: $*" >&2',
      "  exit 2",
      "fi",
  ...
  it("SHA を持たない応答でも、待つのをやめる", () => {
    // **これが本体**（#78）。**レビューできなかった応答**（「環境が無い」通知など）は
    // **SHA を持たない**ので、**既定の出力からは落ちる**——**応答が既に返っているのに
    // 上限まで待つ**ことになり、**同じ周回でゲートへ戻る意味が消える**（#72 の目的）。
    //
    // **「返ったか」と「レビューが成立したか」は別の軸である。** ここが見るのは前者
    withReviews(
      // 既定の出力は動かない（**レビューは成立していない**）が、応答は届いている
      "2026-08-09T00:00:00Z\tabc1234\tlive\n2026-08-09T02:00:00Z\t-\tanswer\n",
    );

    const result = run(["12", "2026-08-09T00:00:00Z"]);

    expect(result.status, "応答が返っているのに待ち続けている").toBe(0);
  });

  it("何を見たか分からない応答でも、待つのをやめる", () => {
    // **記録が `--unknown` だけの PR**（ループの外で作られた PR の初回自動レビュー）では、
    // **👍 は head へ結び付かない**——**落とすと応答が届いているのに上限まで待つ** (#179)。
    // **当たるのは外の人の PR** なので、**外の人ほど待たされる**
    withReviews("2026-08-09T00:00:00Z\tabc1234\tlive\n2026-08-09T02:00:00Z\t-\tack\n");
