...
describe("周回が始まったことを、どう始まったかごと残す", () => {
  ...
  it("誤って cron と記録された 1 本で、物差しが狂わない", () => {
    // **下限（60 秒）は「捨てて呼び直した周回」を落とすために置いてある**が、
    // **実測の誤記録は 231 / 330 / 397 秒**で、**どれもその下限を越えていた**
    // ——**最小を採ると、19 本の正しい記録を 1 本が上書きする**（#601）。
    //
    // **3 作業場とも狂っていた**（**最小 231s / 810s / 1419s、真値は 1800s**）。
    const { dir } = workspace();
    records(dir, [
      [1_000, "cron"],
      [2_800, "cron"],
      ...
      [8_200, "cron"],
      [10_000, "cron"],
      [10_231, "cron"], // 呼び直した周回を cron と記録してしまったぶん
    ]);

    const done = cadence(dir, { LOOP_CADENCE_NOW: "10300" });

    expect(done.stdout, "誤記録 1 本に物差しを持っていかれている").toMatch(/interval=~1800\b/);
  });
  ...
  describe("間隔が渡されていなくても、止まったことに気づく（#438）", () => {
    ...
    // **測るのは、真ん中の間**である（**落とした回も、誤って cron と記録した回も
    // 少数**なので、**真ん中まで届かない**。#601）。
    ...
    it("過去に長く空いた記録があっても、窓を広げない", () => {
      ...
      //
      // **止めるために足したものが、止めなくなる**——**真ん中を採れば、欠落は
      // そこまで届かない**（**ただし、呼び直しの数十秒は周期ではない**ので、下限で外す）。
      const { dir } = workspace();
      records(dir, [
        [1_000, "cron"],
        [2_800, "cron"],
    ...
    it("欠落が過半数なら、正常だと言わない", () => {
      // **中央値は「欠落が少数」を前提にしている**（#606 のレビュー）。
      // **過半数の刻みを落とすと、中央値は真値の整数倍になる**——
      // **窓が倍になり、「予定が半分落ちてから完全に止まる」という
      // この見張りの検出対象そのものが `ok` になる。**
      //
      // **最小は逆向きに外れる**（誤記録 1 本に弱い）ので、**どちらか一方は選べない。**
      // **2 つが違う答えを出す帯は「分からない」**である——**健全へ倒さない。**
      const { dir } = workspace();
      records(dir, [
        [1_000, "cron"],
        [2_800, "cron"], // 1800（真値）
        [6_400, "cron"], // 3600（1 刻み落ちた）
        [10_000, "cron"], // 3600（1 刻み落ちた）
      ]);

      const done = cadence(dir, { LOOP_CADENCE_NOW: "16000" });
    ...
    it("捨てて呼び直した周回で、窓が縮まない", () => {
      ...
      // ——**その間は数十秒のことがある**。**周期として読むと窓が数十秒になり、
      // 正常な周回で鳴り続ける**（**実測の `35 49` がそれ**）。**下限で外す。**
      const { dir } = workspace();
      records(dir, [
        [1_000, "cron"],
 ...
 */
/**
 * **窓の広さは、判定が `ok` の間はどこにも出ていなかった**（#601）。
 *
 * **測ったら、worker-1 の窓は 8989 秒（5.0 周ぶん）まで広がっていた**——
 * **判定は `ok`** なので、**広がっていることを、広がっている間は誰も見られない。**
 * **`age` と比べる相手が出ていない**ためである。
 *
 * **広げた項も出す。** **窓を決めているのが「2 刻み」なのか「直前の間」なのかで、
 * 次の一手が変わる**——**前者なら予定表、後者は記録の欠落**である
 * （**実測: 8758 秒の「間」の中で、cron は 4 回鳴っていた**）。
 *
 * **判定は変えない** (#584)。**出すか出さないかだけ**である。
 */
describe("見張りが緩んでいることを、緩んでいる間に見せる", () => {
  it("窓の広さと、それを決めた項を出す", () => {
    const { dir } = workspace();
    records(dir, [[1_000, "cron"]]);
    ...
    expect(done.stdout, "窓の広さが出ていない").toMatch(/window=3600\(2 刻み\)/);
  });

  it("窓が決まらない行も、同じ形で出す", () => {
    // **行の形を 2 通りにしない**（`AGENTS.md` §5）——**`never` の行だけ
    // 欄が 1 つ足りないと、位置で読む側が黙ってずれる。**
    const { dir } = workspace();
  ...
  it("走っているぶんは、土台の項に重ねて名乗る", () => {
    ...
    // 「直前の間」だったのに、周回の中から見ると `走行中` としか出なかった**）。
    const { dir, stamp } = workspace();
    records(dir, [
      [1_000, "cron"],
      [2_800, "cron"],
    ...
    running(dir, stamp, 11_600);

    const done = cadence(dir, { LOOP_CADENCE_NOW: "11700" });

    expect(done.stdout, "土台の項が隠れている").toMatch(/window=10658\(直前の間\+走行中\)/);
  });

  it("記録の間で広がった窓は、その項を名乗る", () => {
    // **これが worker-1 の形**である——**間隔（1800）ではなく、直前の間（8758）が
    // 窓を決めていた。** **渡されていないときだけ広げる**ので、間隔は渡さない。
    const { dir } = workspace();
    records(dir, [
      [1_000, "cron"],
      [2_800, "cron"],
    ...
    const done = cadence(dir, { LOOP_CADENCE_NOW: "11658" });

    expect(done.stdout, "何が窓を決めたか出ていない").toMatch(/window=10558\(直前の間\)/);
  });
});
