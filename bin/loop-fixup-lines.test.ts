...
describe("要求に応えて作った新しいファイル", () => {
  ...
  it("素で打った人が、どの数字が何かを読める", () => {
    // **3 つの裸の数字は、列ごと取り違えられる**（#553。**同じ人が 2 回**——
    // **#543 の `3 20 13` を「13 行」、#551 の `30 46 0` を「0 行」**）。
    // **意味がスクリプト冒頭のコメントにしか無い**ので、**打った画面には出ない。**
    //
    // **`bin/loop-gate` は最初から言い分けている**ので、**取り違えるのは素で打つとき
    // だけ**である——**そこに出す。**
    const result = runWithFiles(
      [row("src/a.ts", false, 30, 0), row("src/a.test.ts", true, 46, 0)],
      [],
      [],
      [],
    );
    ...
    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout.trim(), "3 列の形が変わっている").toBe("30\t46\t0");
    // **どの数字が何か**を、**数字ごと**出す（**順番の説明だけだと、読み手が
    // もう一度並べ直すことになる**）
    expect(result.stderr, "本体の行数が読めない").toContain("本体 30 行");
    expect(result.stderr, "除外したテストの行数が読めない").toContain("テスト追加 46 行");
    expect(result.stderr, "除外した要求ぶんが読めない").toContain("要求した 0 行");
  });

  it("読む側が見る標準出力には、説明を混ぜない", () => {
    // **`bin/loop-gate` は行ごと正規表現で受ける**（`^([0-9]+)\t([0-9]+)\t([0-9]+)$`）
    // ——**見出しを 1 行足すだけで「手直し量を数値として読めません」で落ちる**。
    // **`bin/loop-fixup-basis` は 3 列の文字列と等値で比べる**（そちらも落ちる）。
    // **足す先は、古い読み手が読まない場所**である（`AGENTS.md` §5）。
    const result = runWithFiles([row("src/a.ts", false, 30, 0)], [], [], []);

    expect(result.stdout, "標準出力に説明が混ざっている").toMatch(/^[0-9]+\t[0-9]+\t[0-9]+\n?$/);
  });
