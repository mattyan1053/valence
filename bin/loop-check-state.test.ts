...
describe("bin/loop-check-state", () => {
  ...
  describe("所要時間を残す（#391）", () => {
    ...
    /**
     * **打つ前に、別の作業場が走っていないかを見る**（#547）。
     *
     * **実測で、単独なら 442〜559 秒、2 本が重なると 1200 秒前後**である
     * ——**両方が同時に遅くなり、道具の 10 分制限を越える。**
     *
     * **`--overlaps` は後から読むもの**で、**打つ前には間に合わない。**
     * **記録は共通ディレクトリの下に揃っている**（**worktree の `.git` も
     * `<共通>/worktrees/<名前>/` にある**）ので、**読むだけで分かる。**
     */
    describe("別の作業場が、いま走っているか", () => {
      /** 別の作業場の記録を置く。**worktree の `.git` は共通ディレクトリの下にある。** */
      function seedElsewhere(
        name: string,
        options: { id: string; lines: string; worktree?: string },
      ): string {
        const gitDir = join(repo, ".git", "worktrees", name);
        mkdirSync(join(gitDir, "valence-check-state.d"), { recursive: true });
        if (options.worktree !== undefined) {
          writeFileSync(join(gitDir, "gitdir"), `${options.worktree}/.git\n`);
        }
        writeFileSync(join(gitDir, "valence-check-state.d", options.id), options.lines);
        return gitDir;
      }

      it("走っている作業場があれば、その場所と経過を出す", () => {
        const started = Math.floor(Date.now() / 1000) - 120;
        // **生きている PID でなければ「走っている」とは言えない**——**自分を使う**
        seedElsewhere("other", {
          id: `${process.pid}`,
          lines: `running\n${started}\n`,
          worktree: "/home/someone/valence-worker-b",
        });

        const listed = run(["--running-elsewhere"]);

        expect(listed.status, "走っているのに無いと言う").toBe(0);
        const [where, elapsed] = (listed.stdout.split("\n")[0] ?? "").split("\t");
        expect(where, "どの作業場か分からない").toBe("/home/someone/valence-worker-b");
        expect(Number(elapsed), "経過が読めない").toBeGreaterThanOrEqual(120);
      });

      it("自分の作業場は数えない", () => {
        // **自分が走っていることは、自分がいちばん知っている**
        // ——**混ぜると、毎回鳴って読まれなくなる**（#248）
        run(["running", `${process.pid}`]);

        expect(run(["--running-elsewhere"]).status, "自分のぶんを数えている").toBe(1);
      });
      ...
      it("終わった走りは数えない", () => {
        ...
        expect(run(["--running-elsewhere"]).status, "終わった走りを数えている").toBe(1);
      });

      it("生きていない走りは数えない", () => {
        // **殺された走りの記録は「走っている」のまま残る**（この口の作り）
        // ——**残骸で毎回鳴らせない**
        seedElsewhere("other", {
          id: "999999999",
        ...
        expect(run(["--running-elsewhere"]).status, "残骸を数えている").toBe(1);
      });

      it("始めた時刻が読めなくても、走っていることは言う", () => {
        // **分からないものを埋めない**が、**黙りもしない**——**重なることが害である**
        seedElsewhere("other", {
          id: `${process.pid}`,
          lines: "running\n",
          worktree: "/home/someone/valence-worker-b",
        ...
        expect(listed.status, "時刻が読めないだけで黙っている").toBe(0);
        expect((listed.stdout.split("\n")[0] ?? "").split("\t")[1], "作り話で埋めている").toBe("-");
      });

      it("場所を引けなければ、記録の在りかで言う", () => {
        // **`gitdir` が読めないことはありうる**——**黙るより、粗くても言う**
        seedElsewhere("other", { id: `${process.pid}`, lines: "running\n1\n" });

        const listed = run(["--running-elsewhere"]);

        expect(listed.status).toBe(0);
        expect((listed.stdout.split("\n")[0] ?? "").split("\t")[0]).toContain("worktrees/other");
      });

      it("どこも走っていなければ、何も言わない", () => {
        expect(run(["--running-elsewhere"]).status).toBe(1);
        expect(run(["--running-elsewhere"]).stdout).toBe("");
      });
    });

    describe("`./task check` を打つ前に、この口を通す（#547）", () => {
      const runner = readFileSync(
        join(fileURLToPath(new URL("..", import.meta.url)), "task"),
        "utf8",
      );
      const looking = runner.slice(runner.indexOf("check_leftovers() {")).split("\n}\n")[0] ?? "";

      it("残りを見る口が、別の作業場も見る", () => {
        // **打つ側は 1 つ打てば済む形にする**——**口が 2 つあると、片方だけ打つ。**
        // **`--overlaps` は後から読むもの**で、**打つ前には間に合わない。**
        expect(looking, "別の作業場を見ていない").toContain("--running-elsewhere");
      });

      it("見つかっても、落とさない", () => {
        // **待つかどうかは打った側が決める**（`bin/loop-check-leftovers` と同じ判断）
        // ——**時間を判定に使わない**（#547 の「踏んではいけないところ」）。
        // **残りを報せる口の合否を、こちらで塗り替えない。**
        expect(looking, "別の作業場の有無で合否が変わる").toContain('return "$status"');
        expect(looking, "走っていることで落としている").not.toMatch(
          /--running-elsewhere[^\n]*\|\|\s*return/,
        );
      });
    });
