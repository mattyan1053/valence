...
describe("bin/loop-gate は 1 条件でも欠ければ止める", () => {
  ...
  const cases: { name: string; env: Record<string, string>; marker: string }[] = [
    ...
    // **exit 5 も別の行にする** (#297)。**落ちた検査と同じ行に見えると、
    // 直せない相手（worker）へ渡し続ける**——**行き先は人である。**
    //
    // **何が欠けているかは書き写さない** (#735 のレビュー)。**`bin/loop-ci-status` が
    // 言う**——**一部だけ欠けている経路が増えた**（#734）**ので、
    // 「1 件も作られていません」と前置きすると、そちらで事実と違う。**
    // **この行が言うのは行き先だけ**である。
    {
      name: "GHA CI: 必須が作られないまま猶予を過ぎた",
      env: { FAKE_CI_EXIT: "5" },
      marker: "人の手が要ります",
    },
...
describe("必須チェックの一覧", () => {
  ...
  it("上書きされたら、そちらを出す", () => {
    // **`main` 側の一覧と同じもの**が出ること。別の口から出すと、
    // **上書きした環境でだけ食い違う**
    //
    // **「判定に使う一覧と同じもの」ではない** (#219)。**PR を見るときの必須は、
    // `main` 側の一覧と PR の head の一覧の**和**である**——**この口は PR 番号を
    // 取らないので、答えられるのは `main` 側だけ**である。
    // **散文を、いま守っているものへ揃えておく**（#221 のレビュー）——
    // **守っていないことを書いたまま残すと、次に読む者はそちらを信じる。**
