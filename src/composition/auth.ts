 ...
 *
 * **5 秒では足りなかった** (#573)。**この機械で実測したら、PR 2 本で 5627 ms**
 * ——**取得は成功しているのに、毎回打ち切られていた。**
 * **利用者には全 PR が「材料がありません」に見え**、**MVP の 3 機能のうち 1 つが
 * 画面から使えない状態**だった（**2026-09-02**）。
 *
 * **1 本あたり 3 往復**（詳細・ファイル・CI）**で、本数ぶん増える**——
 * **2 本で 5.6 秒なので、1 本あたりおよそ 2.8 秒**である。**20 秒なら 7 本前後**まで届く。
 *
 * **正確な値ではない。** **足りなくなったら分かる形にしてある**
