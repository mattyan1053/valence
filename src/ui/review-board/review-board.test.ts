 ...
 * ```
 *
 * **`<span>` は inline** なので、**並べても改行されない。** **区切りは要素の側で
 * 付ける**（**既定で改行されるもの**）——**器のスタイルが当たらない読まれ方でも
 * 繋がらない。**
 *
 * **「見本の CSS に `.flex` も `.flex-col` も無い」と書いていたが、誤りだった**
 * （#710）——**整形された CSS を `.flex{` で探していた**（**本文は `.flex {`**）。
 * **判断は変わらない**（**要素で区切るほうが、スタイルに依存しない**）が、
 * **理由は「CSS が無いから」ではない。**
 *
