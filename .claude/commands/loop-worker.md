...
**記録が無ければ、その場で自分のものになる**（`mine` が決める）。**「自分のものとして
数える」だけにすると、記録の無い PR が 2 本あるとき両方の作業場が止まり、
記録は永久に作られない**——**止まった状態が自分を再生産する**（#184。#238 のレビュー）。
**exit 2 なら、標準エラーに出た内容を報告して終わる。**

**自分のものでない PR は、置き去りにしない。** **持ち主が二度と戻らなくても
`mine` は exit 1 のまま**なので、**そのまま飛ばすと、その PR は誰も直せなくなる**。
**`bin/loop-claim pr` を試す**——**判断はそちらが持っている**（持ち主が走っているか）。

```bash
bin/loop-claim pr <PR番号>
case "$?" in
  0) ;;        # 持ち主は走っていない。**引き継いだ**ので、自分のものとして扱う
  1) ;;        # 別の作業場が直している。**飛ばす**（数えない）
