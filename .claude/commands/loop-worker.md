...
ので、ここに条件を書き写さない。**やり直すのは `cannot lock ref`**——**2 人が同じ周回の
冒頭で fetch すると後から書く側が落ちる**（実測で 25/25）が、**そのとき ref は既に
目的の値**なので、**やり直すと「更新するものが無い」で成功する**（**直っているのに
赤にしない**）。**それ以外の理由では投げ直さない**——**認証で落ちたものは、
何回投げても同じ**である。
