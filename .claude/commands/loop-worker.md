...
次の 3 つでは、GitHub を更新したあとに master へ 1 通送る。

| 送る場面 | 相手にしてほしいこと |
| --- | --- |
| **PR を作った** | ゲートを回す |
| **レビュー指摘に対応して push した** | 対応を確認する |
| **`blocked` にして手を止めた** | 判断する |

**送れなくても手順は変わらない。** 相手が `ListAgents` に居ない、送信に失敗した——
どの場合も、**master は次の周回で GitHub を読んで同じ結論に至る**。
**到達を前提にした書き方をしない。**
...
- **exit 0** → `worker\t<理由>` が出る。**その役へ 1 通送る**（本文は理由をそのまま使ってよい）
