...
**その前に、印を持つ木がここにあるかを見る** (#262 / #369)。**木を触る前に、作業場を
原子的に押さえる**——**読むだけの確認では足りない。**
...
**押さえたことは、進んでよいという意味ではない**——**揃わなければ返して終わる。**

**入る枝が無いなら、`origin/main` へ戻す** (#369)——**ディスクのほうが新しい向き**では、**枝に居る限り呼び直しても揃わない。**

```bash
bin/loop-sync-main >/dev/null                 # **dirty なら落ちる。戻れなければ揃わない**
bin/loop-procedure-stamp worker "<読んだ印>"  # 揃ったか。判定はここ 1 箇所が持つ
```

- **exit 0** → 揃った。**この周回は 1.1 から続ける**——**`before` は戻したあとで読む**
  （**枝の commit と比べると、中身が同じでも「入れ替わった」と出る**）
- **それ以外** → **配られたものが本当に古い**（**入口を触る PR が入った直後**）か、
  **戻せていない**（`bin/loop-stall dirty`）。**返してから、捨てて呼び直す**
...
**握ったまま呼び直さない**（**呼び直した周回が 1.0 で自分自身に阻まれる**）。
...
（**原因は「手順書のほうが新しい」**）——**`git switch --detach origin/main` で揃えて呼び直す。**

**やり直すかどうかは、回数ではなく理由で決まる** (#217)——**判断はスクリプトが持つ**
（一覧の正は `bin/loop-sync-main` の `RETRYABLE_REASONS`）。
