...
**まず、この作業場で周回が走っていないことを見る。** **ここはまだ lease を持っていない**
（取れなかったから来ている）ので、**確かめずに checkout すると、走っている周回の
足元から木を抜く**（#68 の形）。

```bash
bin/loop-lease held worker
```

- **exit 1** → 誰も持っていない。下へ進む
- **exit 0 / それ以外** → **木を触らずに終わる**（走っているか、読めないか）

```bash
bin/loop-claim pr <PR番号>                    # **編集する前に取る** (#203)
case "$?" in                                  # **受けなければ、錠は掛からない**
  0) ;;                                       # 取れた
  1) exit ;;                                  # 別の作業場が直している。**触らない**
  *) exit ;;                                  # 判定できない / 知らない終わり方
esac
gh pr checkout --detach <PR番号>
bin/loop-lease acquire worker "<同じ印>"      # 揃っていれば通る
```

**素通りではない。** **その枝なら手順書もスクリプトも揃う**ので、**噛み合わない
組み合わせで走ることにはならない**——**揃わなければ、また exit 3 で止まる**
（**方向を見て「新しければ通す」のとは別のもの**である）。

**取れたら、この周回は 1.1 で `origin/main` へ移らない**——**`bin/loop-sync-main --fetch-only`
を使う。** **移すと `bin/loop-procedure-changed` が入れ替わりを見つけて周回を捨てる**ので、
**いま入ったばかりの道が、次の行で閉じる。** **やることはその PR の作業**（ステップ 3）である。
