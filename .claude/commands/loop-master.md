...
**配られた手順書が、ディスクより古いことがある** (#241)。**上の判定では見えない**
——**比べているのは git の commit** で、**「いま読んでいるテキストが何版か」は
見ていない**（**切り替えたあとは正しく「変わっていない」と答える**）。

```bash
bin/loop-procedure-stamp master "<この手順書の冒頭にある `版:` の値>"
```

- **exit 0** → 続けてよい
- **exit 1** → **配られたテキストがディスクと違う。上と同じ順で捨てる**
  （`--reset` → 返却 → 呼び直し 1 回）
- **exit 2** → 判定できない。標準エラーに出た内容を報告して終わる

**印が無い版が配られたら、そのまま渡す**（空でよい）——**「古い」側へ倒れる**。
**印を入れる前の版がそれ**である。

**呼び直しても exit 1 なら、そこで終える**（**今回はそれで直らなかった**）。
**`bin/loop-stall procedure-stale` を通す**——**3 周続けば `loop/STOP` が配られ、
人が来る**。**古い版で走り続けるより止まるほうがよい。**
