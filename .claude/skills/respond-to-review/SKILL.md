...
**自分の役を渡す** (#174)。**master と worker は同じ GitHub アカウントで発言する**ので、
**発言そのものに印が無いと、出口は「誰が最後に書いたか」を読めない**——
**worker の返信を master の判断として読むと、当否が判断されていない指摘が
「判断済み」に落ちる**（**通す側へ倒れるので気づけない**）。
**印はスクリプトが付ける**ので、**本文には書かない。**

```bash
# スレッド ID の取り方は、下の「スレッドの resolve」と同じ問い合わせ
bin/loop-review-reply <役> <PR番号> <スレッドID> <本文ファイル>   # 役は master / worker
```
