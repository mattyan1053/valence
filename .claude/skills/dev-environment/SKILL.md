...
**盤面が見せるのはリポジトリの状態**で、**作業場ごとの何かではない**——**認証を通すのは
どの作業場からでも同じ**なので、**ここは既定の作業場でよい。** **ただし、動いているコードは
作業場ごとに違う**（#457）——**盤面の中身を確かめるのは、`origin/main` を映しているほうである。**
...
4. **盤面の中身を確かめるのは、人が見る画面のほう**（`./task loop:preview:show` が URL を出す）
   ——**同じ host で開けば、Cookie はそのまま送られる**ので、**もう一度ログインする必要は無い**
