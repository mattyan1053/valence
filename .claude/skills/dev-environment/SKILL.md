...
**ログインを始めるのは既定の作業場（3000）である。** **許可一覧は port まで固定**
（`supabase/config.toml`。**3000 のみ**）なので、**人が見る作業場（3001 以降）から
始めると、戻り先が当たらない。**

**通したあとの確認は、人が見る画面でよい。** **Cookie は port で分かれない**ので、
**3000 で認証を通せば、人が見る画面にも同じ Cookie が送られる**——**盤面を見るのは、
`origin/main` を映しているほうである**（**worker の作業場は、未コミットか古いものを映す**）。
