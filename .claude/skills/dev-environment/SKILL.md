...
**開くのは、既定の作業場**（`valence`）である。**worker の作業場（3001 番以降）から
ログインすると、`/auth/callback` が許可一覧に当たらない**——**GoTrue は `site_url`
（`http://localhost:3000`）へ落として戻す**ので、**リモートなら止まり、手元なら
「既定の作業場のアプリが動いていれば、黙ってそちらの盤面へ移る」。**

**盤面が見せるのはリポジトリの状態**で、**作業場ごとの何かではない**——**どの作業場の
アプリから見ても同じもの**なので、**既定の作業場から開けばよい。**

**許可一覧は広げない。** **`supabase/config.toml` は開発機どうしで共有**していて（#371）、
**「広げるのはパスだけ。scheme / host / port は固定」**とそこに書いてある——
**約束のほうを変える話**になる。

```bash
# ここは、作業場のある機械で打つ。**既定の作業場（`valence`）で打つこと**
cd <既定の作業場>
./task db:up         # **Supabase も要る**（`./task up` は app しか起こさない）
./task up            # 動いていなければ
# **3000 以外が返ったら、既定の作業場に居ない**
port="$(./task port)"
```
