...
/**
 * ブロックの中で**実際に打つ行**だけを並べる（空行と、行だけのコメントを除く）。
 *
 * **行末のコメントは落とす**——**`bin/loop-lease release worker "<token>"   # …` を
 * 落とすと、いちばん最後の行が見えなくなる。**
 */
function executedLines(block: string): string[] {
  return block
    .split("\n")
...
describe.each(ROLES)("%s の出口", (role) => {
  ...
  it("返す行が、送るブロックのいちばん最後である", () => {
    // **`release > sent` だけでは足りない** (#341 のレビュー 2 周目)——
    // **返す行の後ろへ別の実行行を足しても緑のまま**になる。**手順書が要求しているのは
    // 「いちばん最後」**で、**そこが崩れると、返した直後に入った cron の周回が
    // 残りの出口処理と重なる**（**この PR が消しに来た形そのもの**）。
    //
    // **行そのものと突き合わせる。** **前方一致にすると、`… && gh …` のように
    // 1 行へ足された場合に黙る**——**倒す向きは「最後かどうか分からない」を
    // 「最後である」へ倒さない。**
    const lines = executedLines(sendBlock(role));

    expect(lines.at(-1), "返す行が最後ではない（後ろに実行行がある）").toBe(
      `bin/loop-lease release ${role} "<token>"`,
    );
  });
