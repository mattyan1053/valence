...
第 3 層が数えるのは**まだ誰も見ていない本体の変更行**である（`bin/loop-fixup-lines`）。
テストの追加行と、**レビュースレッドが付いたファイルの本体変更**は数えない。
要求しておいて、そのとおり直した行数で人を呼ぶのは筋が通らない——**よく見たから止まる**が
実際に 4 件中 4 件で起きていた（#126）。ただし**テストの削除行は本体と同じに数える**。
数え方と、その判断の弱点はスクリプトのコメントにある。
