...
## 予定表が切れたことは、外からしか見えない

上の 4 層は**ループが回っていること**を前提にしている。**予定表（cron）が切れると、
周回そのものが来ない**——**`bin/loop-cadence` はそれを読むが、`bin/loop-cadence` を
走らせるのも周回**である。**1 つだけ死ぬなら他の役が見つける**が、**全部が同時に
死ぬと誰も走らない**（**実測: 2026-09-09 13:4x に 3 つの作業場が揃って止まり、
次の周回は 30.6 時間後。人が戻って突くまで誰も気づかなかった**。#670）。

そこで、**GitHub Actions から見る**（`bin/loop-watchdog`。
`.github/workflows/loop-watchdog.yml` が 2 時間ごとに回す）。**VM もセッションも
...
- **予定表を入れ直すのは応急処置である。** **7 日でまた切れる**——**入れ直せるのは
  そのセッション自身だけ**（`CronList` は他のセッションを引けない）
- **切れている窓は、周回のほうで縮める**（#677）——**どちらの役も、1 周に 1 回
  自分の予定表を引き、空のときだけ入れ直す**（**空でなければ足さない**——
  **同じ役の予定が 2 つになると、周回が 2 倍鳴る**）。**入れ直したことは
  `bin/loop-cron-restored` が残し、いまの刻みは `bin/loop-cron-cadence` が
  生きているうちに残す**（**通知で起こされた周回には、運んできた刻みが無い**）。 **周回が 1 つも来なければ入れ直せない**ので、
  **上の見張りの代わりにはならない**
...
**理由の無いまま保留になった PR は、別に見える。** **手順どおりなら作れない**
（**保留する順序は手順書の囲みブロックが持つ**）が、**手で付けたぶんは残る**
——**ステップ 2 は `parked` を選ばない**ので**次の周回は誰も見ない**。
`./task loop:status` と `bin/loop-handoff` が
