**終了コードは、打ったコマンドのもの**——**`$?` はパイプの右や後続の行のものになり、
そちらが成功していれば `0` が返る。** **合否を見るなら先に変数へ受け、繋ぎたいなら
`${PIPESTATUS[0]}` を見る**（#700。**5 箇所に、道具の例つきで在った**——**それでも
同じ日に 2 つの作業場が、どちらもその例には無い道具で踏んだ。** **読む側は、例のほうを
規則として畳む**）。

### 作業場で履歴を引くときは `origin/main`

**作業場の `main` は古いまま残る** (#697)。**どの作業場も `main` をブランチとして
掴まない**（#196）ので、**`bin/loop-sync-main` が動かすのは detached な `HEAD` だけ**
——**ローカルの `main` は、その作業場を作った日のまま止まる。**

**実測**（2026-09-11）: **`main` は 1 か月前**（`2e6c913`）で、**`origin/main` は
その日の先端**（`cfab381`）。**`git log -1 main -- .claude/commands/` は
1 か月前の commit を返した**（**`origin/main` で引くと、その日の #645**）。

**2 人が別のコマンドで踏んでいる**——**`git log main --`**（入口が最後に変わった日を
数えようとした）と、**`git diff main...<枝>`**（PR の差分を見ようとした。
**408 files / 81342 insertions**、実際は 5 files / 802 lines）。**どちらも
気づき方は偶然**である（**中身の食い違い／桁の違和感**）——**もっともらしい範囲に
収まっていたら、そのまま使っていた。**

**`main` という名前は消していない**——**参照は作業場をまたいで共有されている**
（`git rev-parse --git-common-dir`）ので、**消すと人の clone からも消える。**

### 自分の予定表を見る

**予定表（cron）は 7 日で切れる** (#677)。**切れたことに気づく経路は外にある**
（`bin/loop-watchdog`。#670）が、**切れている時間そのものは短くならない**
——**気づいてから誰かが入れ直すまで、止まったまま**である。**周回が来たときに
ここで見れば、切れている窓は「次の周回まで」になる。**

**この節は、どの周回でも必ず 1 回通る場所に置く** (#679 のレビュー)。
**分岐の向こうへ動かさない**——**PR の有無で跳ばす分岐（ステップ 2）の下に置くと、
いちばん静かな盤面**（**`no-work` へ向かう周回、切れたあとに通知で起こされた周回**）
**が素通りする**。**そこがまさに、予定表の切れている窓である。**

**引けるのはセッション自身だけ**である——**`CronList` は他のセッションを引けない**
ので、**ここでしか直せない。**

1. **`CronList` を引く**
2. **空でなければ、いまの刻みを残す**——`bin/loop-cron-cadence <役> <刻み>`。
   **生きている間にしか読めない**（下）
3. **空のときだけ入れ直す**——**残してある刻みで**（`bin/loop-cron-cadence <役>`。
   `/loop <刻み> /loop-<役>` か `CronCreate`）。**空でなければ何もしない**
   ——**足すと同じ役の予定が 2 つになり、周回が 2 倍鳴る**
4. **引けなかったときは足さない**——**判定不能を「無い」へ倒さない**
   （**二重登録のほうが害が大きい**）。**刻みが残っていないときも足さない**
5. **入れ直したら残す**——`bin/loop-cron-restored <刻み>`（**メッセージは揮発する**
   ので、**次に読む人が「なぜ刻みが変わったか」を追えるのは、この記録だけ**である）

**刻みを、生きているうちに残すのはなぜか** (#679 のレビュー 2 周目)。
**通知で起こされた周回は cron に運ばれていない**ので、**「この周回を運んできた刻み」が
無い**——**予定表が切れたあとに起こされた周回こそ入れ直したい**のに、**入れ直す先を
決められない。** **生きている間なら `CronList` が答える**ので、**そのときに残す。**

**これは「窓を縮める」だけ**である。**周回が 1 つも来なければ入れ直せない**
——**全部が同時に切れた場合は、外から気づく側（#670）が受け持つ。**

**重くしない。** **引くのは 1 周に 1 回**で、**外へは出ない。**
...
**理由の無い保留が残ると、誰も見ない**（#163）——**`parked` を選ばないのはこのステップの
決め事**なので、**例外もここに置く**。**手順どおりなら作れない**（**保留する順序は、
保留する側の囲みブロックが持つ**。`AGENTS.md` §5）が、**手で付けたぶんは残りうる。**
...
マージできたら、対応する Issue を閉じる。**自動では閉じない** (#623)——
**PR に閉じる語を書かない**と決めてある（worker の「PR を作る」）ので、
**閉じるのはここだけ**である。**経路を 1 本にしてある**のは、**自動で閉じたものが
この節へ出てこない**からで、**そちらへ流れたぶんは完了条件を誰も読まないまま閉じる**
（**実測: 閉じた 301 件のうち 268 件。うち 235 件は完了条件の節を持っていた**）。

**もともと `Closes` を書かない PR もあった** (#335)。**割った途中の 1 本**や、
**親として残す Issue**がそれで、**完了条件を満たしていても open のまま残る。**
...
gh issue view <N> --comments                 # **持ち越しはここに残っている** (#623)
gh issue close <N> --comment "<何が満たされたか / どの PR で>"
```

**本文だけを読まない** (#623)。**「この条件はまだ人が見ていない」は、周回の申し送りに
書かれていた**——**セッション間のメッセージなので、閉じる側には届かない。**
**届く場所は Issue のコメントである。**
...
まず、**いま自分が閉じた Issue** から外す。**番号で直接指定する。**

```bash
gh issue edit <N> --remove-label in-progress     # label が無くても成功する（冪等）
```

**GitHub に訊かない** (#623)。**自動で閉じないので、「この PR が閉じた Issue」は
GitHub 側に残らない**——**訊くと毎回 0 件が返り**、**さらう側だけが残る。**
**番号は、いま読んで閉じた側が持っている。**
...
**捨てるなら、カウンタを全部消してから終える**——**下のブロックの `--reset` だけは、対象を取らない**。
...
| GHA CI の検査が作られないまま猶予を過ぎた | 下の「検査が作られないまま猶予を過ぎたとき」へ。**人へ渡す** |
...
- **満たされている** → `gh pr edit <PR番号> --remove-label changes-requested` で外す。
  **前へ進んだので、この PR の対応待ちを消す**——`bin/loop-stall --reset "awaiting-worker:<PR番号>@<SHA>"`
  （**`<SHA>` は、いま突き合わせた head**）。次の周回でゲートを回し直す

  **引数無しで打たない** (#566)。**ここはマージしていない**ので、**共有と自分の作業場の
  記録が丸ごと消える**——**続いている別の障害が、3 周に届かなくなる。**

  **消えるのは、いまの head のぶんだけでよい。** **返信だけで満たされた周回は head が
  動かない**ので、**そこが唯一まだ数えている識別子**である。**古い head のぶんは、
  worker が push した時点で別の識別子になっている**（この節の末尾）。
...
**一覧は先に変数へ受ける**——**取得できなかった周回が「レビューが 1 件も無い」と
見分けが付かなくなる**（掃除の段と同じ理由）。
...
決まる**（0 / 1 / 2 で行き先が全部違う）——**受けそこねると、返らなかった周回が
「返った」に化け、判定不能が消える**。
...
**理由の無い保留は、あとから拾われる。** **手順どおりなら作れない**（**投稿が落ちたら
保留にしない**）が、**手で付けたぶんは残る**——**ステップ 2 は `parked` を選ばない**ので、
**次の周回は誰もその PR を見ない**（#163）。**`bin/loop-handoff` が
...
**割って子 Issue を立てるなら、タイトルの末尾を `（#<親の番号>）` にする** (#544)。
**`bin/loop-claim idle` が親と子を突き合わせるのは、ここだけ**である——**枝が名乗るのは
子の番号**なので（`.claude/rules/git-workflow.md`）、**末尾の無い子 Issue を立てると、
親の番号を持つ枝が 1 本も出ない。** **親の時計は `take` のときのまま**なので、
**`IDLE_SEC` を過ぎた時点で必ず「止まっている」に倒れる**（**実測: #540 が 4 時間**）。

**末尾に置く。途中に書いても数えない。** **このリポジトリは理由を厚く書くので、
タイトルにも本文にも番号は出る**——**言及で親子を決めると、開いている PR がある限り
その Issue では黙る**（#322 で 1 度戻している）。

**括弧の中に語を足したものも数えない** (#545 のレビュー)。**数えるのは `（#540）` だけ**で、
**`（#540 の続き）` `（#540 の積み残し）` `（#540 の 1/2）` `（#540 の片側）` はどれも
数えない。**

**これは承知のうえの狭さである。** **末尾が `（#N …）` の Issue は 38 件あり、
厳密に `（#N）` なのは 3 件**である（**master が実データで数えた**）——**残りは
数えられない。** **緩めない理由は、向きが混ざるから**である。

| 末尾 | 何を言っているか | 親子か |
| --- | --- | --- |
| `（#540）` | **#540 を割った先** | **数える** |
| `（#512 の続き）` | #512 のあとに出てきた別の仕事 | 数えない |
| `（#537 で 2 度踏んだ）` | #537 で観測した、という引用 | 数えない |
| `（#82 の前提）` | **#82 がこれを待つ**（**向きが逆**） | 数えない |

**括弧の中を読み分けずに数えると、向きの逆な PR が親を黙らせる**——**#322 が戻した
向き**である。**倒れる向きは安全側**（**誤報が出るだけ**）だが、**書いた人は
「末尾に置いた」と思う**ので、**数えなかったぶんは `bin/loop-claim idle` が標準エラーで
言う**——**呼ばれた人が理由に辿り着ける。**
...
# **先に変数へ受ける**——**取得できなかった周回が「parked は 0 件」と見分けが付かない**
...
# **先に変数へ受ける**——**取得できなかった周回が「0 件」と見分けが付かない**
...
bin/loop-cadence --quiet   # **cron の周回が来ているか** (#530)
bin/loop-handoff master
```

**cron の周回が来ているかを、ここで見る** (#530)。**1 日に 3 回踏み、3 回とも
誰かの気まぐれで見つかった**——**道具は正しく数えていた**（`bin/loop-cadence`）が、
**打つ手順が無かった。**

- **exit 0** → **何も出ない。** そのまま下へ進む（**平常時に鳴る検査は読まれなくなる**。#248）
- **exit 1** → **出た行を読む。** **何が起きているかは、その行の `読み=` にある**
  ——**ここでは決めない**（**原因は 1 つではない**）
  - **`workspace=` が自分の作業場なら**、**このセッションで確かめて直す**
    （**`読み=` が予定表を指しているなら `CronList`。**
    **`:00` / `:30` を外す。recurring は 7 日で期限切れになる**）
  - **自分の作業場でないなら、突く**——**その作業場で走っているセッションへ、
    その行をそのまま 1 通送る**（**他のセッションは、こちらからは確かめられない**）
    - **役では分けない。** **worker は何人居てもよい**（入口 1.0）ので、
      **「他の役なら」で分けると、別の worker の行はどちらにも当たらない**
      ——**`workspace=` は自分のものではなく、役は同じ `worker` である**
    - **これは引き継ぎではない**——**`bin/loop-handoff` が exit 1（渡すものが無い）
      でも送る。** **止まっているかどうかは、渡すものの有無と関係がない**
    - **名前と作業場を結ぶ道具は無い**（**一覧に出るのは名前だけ**）
      ——**結べないときは、出口の 1 通に添える。1 通も無いなら、この周回の報告に出す**
- **exit 2** → **判定できない。** **報告して、そのまま進む**——**止めない**

**「鳴っていない」と「詰まっている」は、外から同じ顔をする** (#531)。
**cron はセッションが暇なときにしか発火しない**ので、**長い周回が続いた作業場は、
予定表が生きていても `stale` に見える**——**実測で、空でない側にこの読みを当てて
外している。** **だからこそ、行の `読み=` をそのまま渡し、こちらで断定しない。**

**`bin/loop-stall` へ積まない。** **引いて確かめた 2 件は、どちらも予定表が空で、
1 回突いて起きた**（**worker-b の 26 時間、master 自身**）。**もう 1 件は空ではなく、
塞がっていただけ**である——**どちらにせよ積まない**：**空なら突けば起きる**し、
**塞がっているなら止めるいわれが無い。** **積むと 3 周で `loop/STOP` が配られ、
生きているループを全部止める。** **突いても起きない形は、まだ踏んでいない**
——**踏んでから足す。**
...
——**倒れる向きはこちらが安い。** **`success` が 1 通も返らなかったときだけ
`--sent` を通さない**（**判断は下の「`--sent` を通すかどうか」が持つ**。**指紋の側は変えない**）。

**落ちたら、その周回のうちに 1 回だけ送り直す** (#293)。**セッション間の送信は失敗する**
——**実測で、名前が合っているのに `Failed to send to …` が返り、直後に同じ宛先で
送り直すと通った**（**1 回の実測である**——**下の「戻り値は届いたかどうかを
言っていない」も読む**）。**記録の側は正しい**ので次の周回が立て直すが、
...
**ただし、戻り値は届いたかどうかを言っていない。** **#293 の根拠は 1 回の実測**
（**落ちた直後に同じ宛先で通った**）**だけ**だったが、**2026-09-14 に逆向きの実測が
3 件出ている。**

- **`Failed to send` が返った 1 通目が、受け手には届いていた**。**受け手の作業場で
  数えた**（**transcript に配送が 1 通**）。
- **`success` が返った 2 通目のほうが、「前のメッセージの繰り返し」として
  相手の inbox で落とされた**。**送信側にはそう見えた**。
- **送り直しが必ず落とされるわけでもない**。**同じ本文が 31 秒あいて 2 通届いた回が
  1 件ある**（**これも受け手の作業場で数えた**）。

**送った戻り値からは、届いたかどうかが分からない。** **`Failed to send` でも相手に渡り、
`success` でも渡らないことがある**——**どちらが起きたかは送信側から見分けられない。**
**受け手なら数えられる**（**配送の時刻が別なら別の配送**）が、**出口の中では受け手を
読めない。** **だから、送り直すかどうかと、`--sent` を通すかどうかを、同じ戻り値から
別々に決める。**

**`--sent` を通すかどうかは、ここだけで決める。**
**上の「`success` が 1 通も返らなかったときだけ」も、下の「送れなかったときは通さない」も、この 2 行を指しているだけ**である。

- **1 通でも `success` が返ったら `--sent` を通す**。**その返り値は「相手の inbox に
  在る」までは言える**——**繰り返しとして弾かれるのは、同じ本文が既に在るときだけ**なので、
  **どちらにせよ相手はその本文を持っている**。
- **どの宛先も `Failed to send` だったら `--sent` を通さない**。**届いているかもしれない
  が、それは分からない**——**分からないときは安い側へ倒す。** **通さずに届いていれば
  次の周回が同じ状態をもう一度立ち上げるだけ**だが、**通して届いていなければ
  二度と送られない**（#258）。

**判定を `bin/loop-handoff` へ寄せない。** **戻り値を見られるのはモデルの turn の中だけ**で、
**スクリプトからは送信の成否が見えない**——**旗で渡したところで、同じ規則が 2 つの場所へ
写るだけ**である（`AGENTS.md` §5）。**判断の 1 箇所は、この 2 行である。**

**2 回目も落ちたら、`--sent` を通さずに終える**（**上の 2 行目のとおり**）。
...
**`--sent` は「知らせた」を記録する口である。** **通さなければ記録は上がらない**ので、
**次の周回で同じ状態がもう一度立ち上がる**（保険が効く）。

**送れなかったときは通さない**（**上の 2 行目**）。**相手が `ListAgents` に居なければ**、
...
**周回が前へ進んだら、必ず次でカウンタを消す。** **消すのは、その周回で解けたぶんだけ。**

```bash
bin/loop-stall --reset <この周回で解けた識別子>
```
...
**積んだ本人でなくてよい**——**前の周回が積んだものでも、いま解けたなら消す**
（要求が満たされたなら `awaiting-worker:<PR番号>@<head>`、というように）。
**解けていないものは残す。**

**引数無しで打つのは 1 箇所だけ** (#566)——**マージしたあと、手順が入れ替わっていて
周回を捨てるとき**である（ステップ 3.1 の「exit 0 — マージする」）。**そこには理由がある**
——**マージはループ全体が前へ進んだ証拠**なので、**どの記録も数え直してよい。**
**ここは違う**——**何を解いたかが分かっている周回**なので、**名指しで消す。**

**走らせて測った** (#566)。**引数無しの `--reset` は、記録を 2 ファイル丸ごと消す**
——**共有（`valence-loop-stall`）と、打った作業場のぶん**である（**4 つ積んで打つと
4 つとも消えた**）。**他の作業場のぶんだけが残る** (#239)。

**worker の空転の節も同じ規則である**（`loop/procedure/worker.md`）。
**worker には打ち切りの例外が無い**——**マージしないので、その場面が来ない。**
...
- **印がずれた段でカウンタを消さないのは**、**消すと次の周回が積んだ
