  ...
  **コメントして終わってはいけない。** `in-progress` が残ったままだと次の周回も
  ...
  - **OPEN** → **`parked` かどうかを見る前に、持ち主を確かめる** (#503)。
    ...
    **`parked` の分岐が先にあったころは、`resume` が Issue の記録をこちらへ
    ...
    - **`parked` が付いている** → master が保留にした PR である。**何もしないで
      ステップ 4 へ進む。** 先行 Issue を実装させるための保留なので、ここで止まると
      **PR-B を作れず保留の意味が消える**。label は `in-progress` のままでよい
      （再開するのは master の指示を受けてから）
