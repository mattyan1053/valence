...
describe("ずれたときの行き先が、手順書に書いてある", () => {
  for (const role of ROLES) {
    it(`${role} の手順書が、入口で印を渡している`, () => {
      ...
      // **この経路でカウンタを消さない** (#243 のレビュー)。**消すと、次の周回が
      ...
      // **禁止にコマンドの形を残さない** (#566 / #569 のレビュー)——**散文に裸の
      // `bin/loop-stall --reset` が出ていると、読んだ側はそれを打つ**
      // （`loop/stall-reset-scope.test.ts`）。**言い回しはそちらへ合わせる。**
      expect(section, "消さない理由が書いていない").toMatch(/カウンタを消さない/);
    });
  }
  ...
  for (const role of ROLES) {
    it(`${role} は、--reset が落ちたときの行き先を書いている`, () => {
      ...
      // **止めない形にしたぶん、気づける道は報告しか残っていない**（#272 のレビュー）。
      // **1 と 2 は「止まらない」ことの説明**で、**「消えなかったカウンタが残っている」を
      // 人に届けるのは、この一文だけ**である——**落ちると、この経路は黙って先へ進む。**
      expect(section, "報告することが書いていない").toMatch(/報告して.{0,10}先へ進む/);
    });
  }
