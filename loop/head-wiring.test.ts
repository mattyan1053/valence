...
describe("周回の途中で動く head", () => {
  ...
  it("評価に基づいて書く節を、全部並べて突き合わせる", () => {
    ...
    expect(writingSections()).toEqual([
      // **保留を外す前に、head が動いたことを確かめる**（#70）——
      // **評価していない head の錠を外さない**、はここでも同じである
      ["## 2. open PR を見て、見る順番を決める", "確かめてから書く"],
      // **マージだけは GitHub 側が同じことを確かめる**（`--match-head-commit`）。
      // **#145 の本文が手本にした手当てそのもの**なので、二重には置かない
      ["### exit 0 — マージする", "確かめない"],
      // **CI が決着しないまま予算を超えた**（#206）。**動いていたら、その pending は
      // 前の head のもの**である——**新しい head では検査が作り直される**ので、
      // **人を呼ぶ理由が無い**
      ["#### 予算を超えたとき", "確かめてから書く"],
      // **必須チェックが作られないまま猶予を過ぎた**（#297 / #734）。**予算を超えたときと
      // 同じで、head が動いていれば検査は作り直される**——**人を呼ぶ理由が無い**
      ["#### 検査が作られないまま猶予を過ぎたとき", "確かめてから書く"],
      ...
      ["#### human — 人を呼ぶ", "確かめてから書く"],
      // **指摘 0 件で、第 3 層だけがゲートを塞いでいる**（#324）。**頼むのは
      // 取り込み直しだけ**だが、**評価した head に対して頼んでいる**——
      // **動いていたら、その依頼はもう要らないかもしれない**
      ["#### recount — main を取り込み直して、レビューを数え直す", "確かめてから書く"],
