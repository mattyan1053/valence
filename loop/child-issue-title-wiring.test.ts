...
describe("子 Issue のタイトルの末尾", () => {
  ...
  it("括弧の中に語を足した形を、両方の文書が名指しで断っている", () => {
    // **末尾に置けば数えられる、と読める規則だった** (#545 のレビュー)。**実データでは
    // 末尾が `（#N …）` の Issue が 38 件あり、厳密に `（#N）` なのは 3 件**である
    // ——**書いた人は「末尾に置いた」と思う。** **狭いことを、狭いと書く。**
    for (const path of ["loop/procedure/master.md", ".claude/rules/git-workflow.md"]) {
      const rule = lineWith(read(path), "括弧の中に語を足したものも数えない");

      expect(rule, `${path} が実例を挙げていない`).toBeTruthy();
    }
    // **実例が要る**——**「語を足したもの」だけでは、どれが当たるか分からない**
    expect(read("loop/procedure/master.md"), "数えない実例が無い").toContain("（#540 の続き）");
  });
