...
describe("停止識別子", () => {
  ...
  it("どの節で何を打つかを、列挙して突き合わせる", () => {
    ...
    expect(identifiersWithSection("master")).toEqual([
      ...
      // **マージした瞬間に閉じる Issue を、先に読む**（#623）。**読めなかったら
      // マージしない**——**「閉じる参照は無い」と混ぜると、読めなかった周回が
      // 自動で閉じる側へ倒れ**、**完了条件を読む経路へ二度と来ない**
      ["### exit 0 — マージする", "pr-lookup-failed"],
      ...
      // **節の名前は「1 件も」ではなくなった** (#735 のレビュー 2 周目)
      // ——**一部だけ欠けたまま猶予を過ぎる経路が増えた**（#734）。
      // **識別子は変えていない**——**`ci-missing` は「CI が欠けている」**で、
      // **どちらの経路でもその状態を言っている**（**変えると、積んだぶんが数え直しになる**。#185）
      ["#### 検査が作られないまま猶予を過ぎたとき", "ci-missing:<PR番号>@<SHA>"],
      ...
      // **数える側が落ちたときも同じ**（#318）。**`bin/loop-parked-issues` が
      // 「読めない」と言ったのを 0 件へ倒すと、保留の枠が引かれないまま先へ進む**
      ["## 6. 着手順を決める（`ready` を 2 件までに保つ）", "pr-lookup-failed"],
      ["## 6. 着手順を決める（`ready` を 2 件までに保つ）", "too-many-ready:<件数>"],
      // **着手できる open PR を数えるための一覧**（#546）。**取れなかったのを
      // 0 件へ倒すと、人待ちでもないのに「尽きた」と数える**
      ["### 作業が尽きたとき", "pr-lookup-failed"],
      // **数える側が落ちたときも同じ**（`bin/loop-open-work`）
      ["### 作業が尽きたとき", "pr-lookup-failed"],
      // **着手中の Issue 番号**（#558）。**取れなかったのを 0 件へ倒すと、
      // 人待ちの PR を持つ Issue が数から消え、進められる作業があるのに尽きたと数える**
      ["### 作業が尽きたとき", "issue-lookup-failed"],
      // **数える側が落ちたときも同じ**（`bin/loop-in-progress-work`）
      ["### 作業が尽きたとき", "issue-lookup-failed"],
      ["### 作業が尽きたとき", "no-work"],
      ...
      ["### 周回の出口", "issue-lookup-failed"],
      // **枝の上で終えると、次の周回が必ず捨てられる**（#405）。**master は枝へ
      // 移らない**ので**普段は何も起きない**が、**手順を 2 つに分けると片方だけ
      ...
      ["### 配られた本文が古いとき、自分で読みに行く (#373)", "procedure-stale"],
    ]);
  });
